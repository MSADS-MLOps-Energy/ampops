...
| Experiment tracking + registry | **MLflow** (originally local/self-hosted, not the managed service — superseded, see below) | Free, no account/API key friction (unlike W&B), and the Model Registry is built in — one tool covers both tracking and registry requirements. `mlflow ui` gives you the dashboard screenshot the rubric wants with zero extra setup. **Superseded:** the project later adopted Databricks MLflow/Unity Catalog as the default tracking/registry backend once workspace access was available, with local self-hosted MLflow kept as a documented fallback rather than the primary. See `docs/databricks_experiment_tracking.md`. |
